...
# Create a new contact out of an email address.  Tries to
# include the viblio user if they exist.
#
sub create_contact :Private {
    my( $self, $c, $email ) = @_;
    my $user = ( $c->user->can( 'obj' ) ? $c->user->obj : $c->user );

    my $contact = $user->find_or_create_related
	( 'contacts', { contact_email => $email } );

    if ( ! $contact->contact_viblio_id ) {
	my $viblio_user = $c->model( 'RDS::User' )->find({ email => $email });
	if ( $viblio_user ) {
	    $contact->contact_viblio_id( $viblio_user->id );
	    $contact->contact_name( $viblio_user->displayname() );
	    $contact->update;
	}
    }
    unless( $contact->contact_name ) {
	my $a = $self->is_email_valid( $contact->contact_email );
	$contact->contact_name( $a->name );
	$contact->update;
    }
    return $contact;
}

# Create a new group.  Takes an optional list of emails, which can
# be a comma delimitted string, or an array.
#
sub create_group :Private {
    my( $self, $c, $name, $list ) = @_;

    my $user = ( $c->user->can( 'obj' ) ? $c->user->obj : $c->user );

    my $group = $c->model( 'RDS::Contact' )->find_or_create({
	user_id => $user->id,
	is_group => 1,
	contact_name => $name });
    unless( $group ) {
	...
	$c->model( 'RDS::ContactGroup' )->find_or_create({
	    group_id => $group->id,
	    contact_id => $contact->id });
	push( @display_names, $contact->contact_name );
    }

    unless( $group->contact_name() ) {
	if ( $#display_names == -1 ) {
	    $group->contact_name( 'Unnamed Group' . $group->id );
	    $group->update;
	}
	elsif ( $#display_names == 0 ) {
	    $group->contact_name( $display_names[0] . ' Group' );
	    $group->update;
	}
	elsif ( $#display_names == 1 ) {
	    $group->contact_name( join( ', ', @display_names ) );
	    $group->update;
	}
	elsif ( $#display_names > 1 ) {
	    $group->contact_name( $display_names[0] . ', ' .
