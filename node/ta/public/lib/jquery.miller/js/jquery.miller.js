(function($) {
	...
	if (methods[mixed] ) {
	    ...
	    var showHideCategory = function( evt ) {
		var self = this;
		var cat = $(self).data( 'cname' );
		$(self).siblings('li[data-cat="'+cat+'"]').toggle();
		$(self).find('i').toggleClass("icon-caret-right icon-carent-down");
	    };
		...
		if (lines == null) {
			...
			$.each(lines, function(id, data) {
				...
				var line = $('<li>')
				    ...
				    .appendTo(column);
				line.append( '<i class="icon-caret-down"></i>' )
